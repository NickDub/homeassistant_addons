...
| Welcome | HA Version | CPU | Memory | Storage | Network | Exit Screen |Stats Screen* |
|-----------|-----------|-----------|-----------|---------------|---------------|---------------|---------------|
| ![Welcome][welcome-url] | ![Version][version-url] | ![CPU Stats][cpu-stats-url] | ![RAM Stats][ram-stats-url] | ![Storage Stats][storage-stats-url] | ![Network Stats][network-stats-url] | ![Exit][exit-url] | ![Stats][stats-url] |
...
======================
This repository has been broken out to work as a standalone service and will work on a standard Raspberry Pi running Raspian. Any screens which are dependent on Home Assistant (e.g. ```Version```) will be automatically disabled.

The Home Assistant add-on that uses this can be accessed from [HomeAssistant_Addons](https://github.com/NickDub/homeassistant_addons)
<br>
<br>
...
```
curl -sSL https://raw.githubusercontent.com/NickDub/rpi_i2c_oled/v1.0.4/basic-install.sh | sudo bash
```
...
Checkout this code
```
git clone git@github.com:NickDub/rpi_i2c_oled.git
cd rpi_i2c_oled
```
...
| Show_Version_Screen  | boolean | **Required** | Show the Home Assistant core and version screen. I you're not using Home Assistant, leave this false         | `false`              |
...
[version-url]: /img/examples/version.png?raw=true
