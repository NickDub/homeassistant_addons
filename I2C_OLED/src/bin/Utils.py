...
class Utils:
    ...
    @staticmethod
    def get_uptime():
        return str(Utils.shell_cmd("uptime | grep -ohe 'up .*' | sed 's/,//g' | awk '{ print $2\" \"$3 }'")).strip()
    ...
    @staticmethod
    def get_memory_info():
        """returns memory stats as a comma-separated string: used, total, used%, free%"""
        return Utils.shell_cmd("free -h | awk 'NR==2{printf \"%.1f,%.1f,%.1f,%.1f\", $3, $2, 100*$3/$2, 100*($2-$3)/$2}'")

    @staticmethod
    def get_storage_info(drive = "/"):
        """returns storage stats as a comma-separated string: used, total, used%, free%"""
        return Utils.shell_cmd('df -h '+drive+' | awk \'$NF=="/"{printf "%d,%d,%.1f,%.1f", $3, $2, 100*$3/$2, 100*($2-$3)/$2}\'')
